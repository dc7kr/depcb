...
PcbKeyBinding	key_binding[] = {
	...
	{ GDK_KEY_r,	redo,			0,
	  "r\tRedo\n" },
	{ GDK_KEY_s,	change_mode,		PCB_SELECT,
	  "s\tEnter select mode: click points to select\n" },
	{ GDK_KEY_t,	change_mode,		PCB_TRACE,
	  "t\tEnter trace mode: click points and change layers to add continuous line\n" },
	{ GDK_KEY_u,	undo,			0,
	  "u\tUndo\n" },
	{ GDK_KEY_v,	change_mode,		PCB_ADD_VIA,
	  "v\tEnter add via mode: click to add a point on all layers\n" },
	{ GDK_KEY_w,	save_project,		0,
	  "w\tWrite (save) project\n" },
	{ GDK_KEY_x,	toggle_overlays,	0,
	  "x\tShow/hide overlays (you can't do much with overlays hidden)\n" },
	{ GDK_KEY_plus,	zoom,			ZOOM_IN,
	  "+\tZoom in\n" },
	{ GDK_KEY_minus,	zoom,			ZOOM_OUT,
	  "-\tZoom out\n" },
	{ GDK_KEY_0,	zoom,			ZOOM_NORMAL,
	  "0\tZoom to 100%\n" },
	{ GDK_KEY_Up,	switch_layer,		UP,
	  "Up\tGo up a layer\n" },
	{ GDK_KEY_Down,	switch_layer,		DOWN,
	  "Down\tGo down a layer\n" },
	{ GDK_KEY_Home,	switch_layer,		TOP,
	  "Home\tGo to first layer\n" },
	{ GDK_KEY_End,	switch_layer,		BOTTOM,
	  "End\tGo to last layer\n" },
	{ GDK_KEY_Escape,	deselect_all,		0,
	  "Esc\tDeselect all\n" },
	{ 0,		NULL,			0,
	  NULL },
};
